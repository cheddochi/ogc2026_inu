...
Strategy:
    Keep trusted v085 as the default path, then replay a tiny bounded
    reinsertion portfolio on the 4-bay early-short low-proc diffuse family.
...
Metadata:
    ...
    status: candidate
    timestamp: 2026-06-19 06:44 KST
    strategy:
        - Preserve v085 unchanged outside the target family.
        - Build the trusted v085 warm start first.
        - On the low-proc diffuse 4-bay family, replay a bounded one-block
          reinsertion over a short tardy shortlist.
        - Keep only strictly better officially feasible results.
    hypothesis:
        The current warm start still leaves small local placement slack on the
        early-short low-proc diffuse 4-bay family. One extra bounded tardy
        reinsertion can recover objective on several rows without touching the
        rest of the training set.
    intended_metric_target:
        - improve low-proc diffuse 4-bay family rows
    ...
    benchmark_evidence_path:
        pending
    rollback_target: reboot_v085_20260619_0512_fourbay_dense_extended_reinsert
"""
...
ACTIVE_VERSION = "reboot_v086_20260619_0644_fourbay_lowproc_diffuse_reinsert"
...
def _matches_fourbay_lowproc_diffuse_family(features: dict[str, float]) -> bool:
    return (
        int(features["bays"]) == 4
        and 200 <= int(features["blocks"]) <= 300
        and 7.2 <= features["proc_mean"] <= 7.9
        and 0.25 <= features["pref_concentration"] <= 0.31
        and 0.25 <= features["pref_pressure"] <= 0.29
        and features["workload_imbalance_pressure"] <= 0.13
        and 1.2 <= features["slack_mean"] <= 1.7
    )
...
def _try_lowproc_diffuse_reinsert_portfolio(
    prob_info: dict,
    base_solution: dict,
    base_result: dict,
    remaining: float,
    ...
    print(
        f"[baseline_hh reboot_v086] fourbay_lowproc_diffuse_reinsert instance={prob_info.get('name')} "
...
def algorithm(prob_info: dict, timelimit: float = 60) -> dict:
    ...
    base_solution = v085.algorithm(prob_info, timelimit)
    base_result = v064.v001.check_feasibility(prob_info, base_solution)
    if (
        not base_result.get("feasible")
        or not _matches_fourbay_lowproc_diffuse_family(features)
    ...
    if remaining <= 0.4:
        print(
            f"[baseline_hh reboot_v086] skip_fourbay_lowproc_diffuse_reinsert instance={prob_info.get('name')} "
            f"tier={tier} remaining={remaining:.2f}s"
        )
        return base_solution

    research_solution, research_result = _try_lowproc_diffuse_reinsert_portfolio(
        prob_info,
        base_solution,
        base_result,
        remaining,
    ...
    if v064._result_key(research_result) < v064._result_key(base_result):
        print(
            f"[baseline_hh reboot_v086] selected_fourbay_lowproc_diffuse_reinsert instance={prob_info.get('name')} "
            f"T={research_result.get('obj1')} objective={research_result.get('objective')}"
        )
        return research_solution

    print(
        f"[baseline_hh reboot_v086] keep_warm_start instance={prob_info.get('name')} "
