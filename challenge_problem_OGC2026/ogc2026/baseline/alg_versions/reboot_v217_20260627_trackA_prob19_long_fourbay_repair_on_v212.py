"""reboot_v217_20260627_trackA_prob19_long_fourbay_repair_on_v212.py

Strategy:
    Preserve the trusted v212 Track A portfolio and runtime-cliff ordering, and
    add one extra bounded specialist inside the stable four-bay Track A
    portfolio for the long `300-block / 4-bay / tight-slack` subtype.
    The new candidate reuses the cheap one-block quantile reinsertion kernel
    after multiblock plus stable-window repair, aiming at the residual
    `prob_19`-like first20 backlog without replaying the rejected
    `prob_13` / `prob_14` exact slice.
"""
...
def _reserved_specialist_budget(
    ...
    if current_t < 150.0:
        return 0.0
    if not (
        _allow_mid_fourbay_tardy_repair(features) or _allow_micro_pair_prefix(features)
    ):
        return 0.0
    base = {
        "standard": 2.4,
