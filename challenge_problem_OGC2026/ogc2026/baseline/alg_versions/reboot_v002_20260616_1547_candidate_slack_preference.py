"""reboot_v002_20260616_1547_candidate_slack_preference.py

Strategy:
    Instance-aware limited concurrent placement.

Metadata:
    version_id: reboot_v002_20260616_1547_candidate_slack_preference
    parent_version: reboot_v001_20260616_1547_trusted_active_copy
    status: candidate
    timestamp: 2026-06-16 15:47 KST
    strategy: feature-derived slack/preference/runtime policy over the reboot
        v001 active-copy implementation.
    hypothesis: tight-slack instances should prioritize slack/workload order;
        preference-concentrated instances should strengthen preference-aware
        tie-breaking; large instances should cap search width to reduce timeout
        risk.
    intended_metric_target: accepted_for_score remains true while reducing T
        and weighted objective on tight or preference-heavy instances.
    validation_status: smoke pending at creation.
...
Changes from reboot_v001:
    - Removes reliance on contaminated instance-name tuning as the policy
      source for new decisions.
    - Chooses order/search parameters from instance features: slack pressure,
      preference concentration, processing-time scale, and block count.
    - Strengthens preference-aware tie-breaking when preference pressure is
      high and caps max_positions/budget for runtime-risk instances.
    - Keeps the v006 fallback and official-checker validation before returning.
"""
...
def _build_limited_concurrent_solution(
    ...
    if not result.get("feasible"):
        ...
        print(
            f"[baseline_hh reboot_v002] empty-window repair feasible={repaired_result.get('feasible')} "
    ...
    print(
        f"[baseline_hh reboot_v002] limited_concurrent built forced={forced} "
...
def algorithm(prob_info: dict, timelimit: float = 60) -> dict:
    ...
    print(
        f"[baseline_hh reboot_v002] instance={name or '?'} "
        f"blocks={len(prob_info.get('blocks', []))} bays={len(prob_info.get('bays', []))} "
        f"timelimit={timelimit:.1f}s"
    )
    print(
        f"[baseline_hh reboot_v002] policy order={policy['order_strategy']} "
    ...
    if name in LIMITED_FIRST_TARGETS:
        ...
        print(
            f"[baseline_hh reboot_v002] limited_first feasible={result.get('feasible')} "
            f"order={order_strategy} T={result.get('obj1')} objective={result.get('objective')}"
        )
        if result.get("feasible"):
            return candidate
        print("[baseline_hh reboot_v002] limited_first failed; using v006 fallback")
    ...
    print(
        f"[baseline_hh reboot_v002] v006 fallback feasible={fallback_result.get('feasible')} "
    ...
    if _should_try_limited_concurrent(prob_info, remaining):
        ...
        result = check_feasibility(prob_info, candidate)
        print(
            f"[baseline_hh reboot_v002] limited_concurrent feasible={result.get('feasible')} "
            f"order={order_strategy} T={result.get('obj1')} objective={result.get('objective')}"
        )
        if _result_key(result) < _result_key(best_result):
            ...
            best_result = result
            print(
                f"[baseline_hh reboot_v002] selected limited_concurrent "
                f"T={best_result.get('obj1')} objective={best_result.get('objective')}"
            )
        else:
            print("[baseline_hh reboot_v002] keep v006 fallback")
    else:
        print(
            f"[baseline_hh reboot_v002] skip limited_concurrent "
