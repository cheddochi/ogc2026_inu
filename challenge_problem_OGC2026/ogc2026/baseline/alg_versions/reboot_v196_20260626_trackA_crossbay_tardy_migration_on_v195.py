"""reboot_v196_20260626_trackA_crossbay_tardy_migration_on_v195.py

Strategy:
    Keep trusted v195 as the fallback surface, then add a bounded 4-bay Family A
    cross-bay tardy migration pass for the residual first20 backlog.
"""
...
ACTIVE_VERSION = "reboot_v196_20260626_trackA_crossbay_tardy_migration_on_v195"
...
def algorithm(prob_info: dict, timelimit: float = 60) -> dict:
    ...
    if spendable <= 1.0:
        print(
            f"[baseline_hh reboot_v196] skip_trackA_crossbay instance={prob_info.get('name')} "
    ...
    print(
        f"[baseline_hh reboot_v196] trackA_crossbay instance={prob_info.get('name')} "
    ...
    if v186.v064._result_key(best_result) < v186.v064._result_key(base_result):
        print(
            f"[baseline_hh reboot_v196] selected_trackA_crossbay instance={prob_info.get('name')} "
