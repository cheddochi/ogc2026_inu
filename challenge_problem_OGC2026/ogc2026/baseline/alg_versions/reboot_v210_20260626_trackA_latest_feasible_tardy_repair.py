"""reboot_v210_20260626_trackA_latest_feasible_tardy_repair.py

Strategy:
    Preserve the trusted v207 Track A portfolio and runtime-cliff ordering, but
    add one more bounded candidate for mid-size four-bay high-urgency rows:
    apply a very small tardy reinsertion / empty-window repair pass on top of
    the selected warm start.
"""
...
def _try_mid_fourbay_tardy_repair(
    ...
    for _ in range(2):
        ...
        for block_id in shortlist:
            ...
            print(
                f"[baseline_hh reboot_v210] mid_fourbay_tardy_candidate instance={prob_info.get('name')} "
...
def _runtime_cliff_ordered_candidates(
    ...
    if (
        ...
        if spendable > 1.0:
            ...
            print(
            f"[baseline_hh reboot_v210] runtime_window_candidate instance={prob_info.get('name')} "
            ...
            if v186.v064._result_key(window_result) < v186.v064._result_key(best_result):
                ...
                print(
                    f"[baseline_hh reboot_v210] selected_runtime_window_candidate instance={prob_info.get('name')} "
                    f"T={best_result.get('obj1')} objective={best_result.get('objective')}"
                )
        else:
            print(
                f"[baseline_hh reboot_v210] skip_runtime_window instance={prob_info.get('name')} "
        ...
        print(
            f"[baseline_hh reboot_v210] return_runtime_base instance={prob_info.get('name')} "
    ...
    if remaining > reserve + deep_fallback_budget:
        ...
        print(
            f"[baseline_hh reboot_v210] runtime_deep_fallback instance={prob_info.get('name')} "
        ...
        if v186.v064._result_key(fallback_result) < v186.v064._result_key(best_result):
            ...
            print(
                f"[baseline_hh reboot_v210] selected_runtime_deep_fallback instance={prob_info.get('name')} "
                f"T={best_result.get('obj1')} objective={best_result.get('objective')}"
            )
    else:
        print(
            f"[baseline_hh reboot_v210] skip_runtime_deep_fallback instance={prob_info.get('name')} "
            f"tier={tier} remaining={remaining:.2f}s reserve={reserve:.2f}s "
...
def _stable_fourbay_internal_portfolio(
    ...
    if spendable <= 1.0:
        print(
            f"[baseline_hh reboot_v210] skip_stable_fourbay_portfolio instance={prob_info.get('name')} "
            f"tier={tier} remaining={remaining:.2f}s reserve={reserve:.2f}s "
    ...
    print(
        f"[baseline_hh reboot_v210] multiblock_candidate instance={prob_info.get('name')} "
    ...
    if v186.v064._result_key(multiblock_result) < v186.v064._result_key(best_result):
        ...
        print(
            f"[baseline_hh reboot_v210] selected_multiblock_candidate instance={prob_info.get('name')} "
            f"T={best_result.get('obj1')} objective={best_result.get('objective')}"
        )
    ...
    if spendable > 1.0 and v195._allow_window_reorder(features):
        ...
        print(
            f"[baseline_hh reboot_v210] stable_window_candidate instance={prob_info.get('name')} "
        ...
        if v186.v064._result_key(window_result) < v186.v064._result_key(best_result):
            ...
            best_result = window_result
            print(
                f"[baseline_hh reboot_v210] selected_stable_window_candidate instance={prob_info.get('name')} "
                f"T={best_result.get('obj1')} objective={best_result.get('objective')}"
            )
    else:
        print(
            f"[baseline_hh reboot_v210] skip_stable_window_candidate instance={prob_info.get('name')} "
            f"tier={tier} remaining={remaining:.2f}s reserve={reserve:.2f}s "
            f"T={best_result.get('obj1')} objective={best_result.get('objective')}"
        )
    ...
    if (
        ...
        print(
            f"[baseline_hh reboot_v210] mid_fourbay_tardy_repair instance={prob_info.get('name')} "
        ...
        if v186.v064._result_key(residual_result) < v186.v064._result_key(best_result):
            ...
            print(
                f"[baseline_hh reboot_v210] selected_mid_fourbay_tardy_repair instance={prob_info.get('name')} "
                f"T={best_result.get('obj1')} objective={best_result.get('objective')}"
            )
    else:
        print(
            f"[baseline_hh reboot_v210] skip_mid_fourbay_tardy_repair instance={prob_info.get('name')} "
            f"tier={tier} remaining={remaining:.2f}s reserve={reserve:.2f}s "
            f"T={best_result.get('obj1')} objective={best_result.get('objective')}"
        )
...
def algorithm(prob_info: dict, timelimit: float = 60) -> dict:
    ...
    if _is_runtime_cliff_tightslack(features):
        print(
            f"[baseline_hh reboot_v210] route_runtime_cliff instance={prob_info.get('name')} "
    ...
    if (
        ...
        print(
            f"[baseline_hh reboot_v210] return_portfolio instance={prob_info.get('name')} "
    ...
    if _is_stable_fourbay_tightslack(features):
        print(
            f"[baseline_hh reboot_v210] route_stable_fourbay instance={prob_info.get('name')} "
            f"blocks={int(features.get('blocks', 0))} bays={int(features.get('bays', 0))}"
        )
    ...
    if spendable <= 1.0:
        print(
            f"[baseline_hh reboot_v210] skip_default_multiblock instance={prob_info.get('name')} "
            f"tier={tier} remaining={remaining:.2f}s reserve={reserve:.2f}s "
            f"T={portfolio_result.get('obj1')} objective={portfolio_result.get('objective')}"
        )
        return portfolio_solution
    ...
    print(
        f"[baseline_hh reboot_v210] default_multiblock instance={prob_info.get('name')} "
    ...
    if v186.v064._result_key(repaired_result) < v186.v064._result_key(portfolio_result):
        print(
            f"[baseline_hh reboot_v210] selected_default_multiblock instance={prob_info.get('name')} "
