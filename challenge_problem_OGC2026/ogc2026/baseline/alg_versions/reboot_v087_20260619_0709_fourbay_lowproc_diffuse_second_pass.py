...
Strategy:
    Keep trusted v086 as the default path, then replay one more bounded
    reinsertion pass on the same 4-bay early-short low-proc diffuse family.
...
Metadata:
    ...
    status: candidate
    timestamp: 2026-06-19 07:09 KST
    strategy:
        - Preserve v086 unchanged outside the target family.
        - Build the trusted v086 warm start first.
        - On the same low-proc diffuse 4-bay family, replay one extra bounded
          tardy reinsertion pass on the updated assignments.
        - Keep only strictly better officially feasible results.
    hypothesis:
        The first reinsertion pass from v086 improved the family but did not
        fully saturate it. One more pass over the updated tardy shortlist can
        recover the remaining objective on a subset of the same rows.
    ...
    benchmark_evidence_path:
        pending
    rollback_target: reboot_v086_20260619_0644_fourbay_lowproc_diffuse_reinsert
"""
...
from alg_versions import reboot_v086_20260619_0644_fourbay_lowproc_diffuse_reinsert as v086


ACTIVE_VERSION = "reboot_v087_20260619_0709_fourbay_lowproc_diffuse_second_pass"
...
def _try_second_pass_portfolio(
    prob_info: dict,
    base_solution: dict,
    base_result: dict,
    remaining: float,
    ...
    print(
        f"[baseline_hh reboot_v087] fourbay_lowproc_diffuse_second_pass instance={prob_info.get('name')} "
...
def algorithm(prob_info: dict, timelimit: float = 60) -> dict:
    ...
    started = time.time()
    features = v086._selector_features(prob_info)
    tier = v064.v050._time_tier(float(timelimit))

    base_solution = v086.algorithm(prob_info, timelimit)
    base_result = v064.v001.check_feasibility(prob_info, base_solution)
    if (
        not base_result.get("feasible")
        or not v086._matches_fourbay_lowproc_diffuse_family(features)
    ...
    if remaining <= 0.4:
        print(
            f"[baseline_hh reboot_v087] skip_fourbay_lowproc_diffuse_second_pass instance={prob_info.get('name')} "
            f"tier={tier} remaining={remaining:.2f}s"
        )
        return base_solution

    research_solution, research_result = _try_second_pass_portfolio(
        prob_info,
        base_solution,
        base_result,
        remaining,
    ...
    if v064._result_key(research_result) < v064._result_key(base_result):
        print(
            f"[baseline_hh reboot_v087] selected_fourbay_lowproc_diffuse_second_pass instance={prob_info.get('name')} "
            f"T={research_result.get('obj1')} objective={research_result.get('objective')}"
        )
        return research_solution

    print(
        f"[baseline_hh reboot_v087] keep_warm_start instance={prob_info.get('name')} "
