...
Strategy:
    Keep trusted v142 as the default path, then apply a tiny second-pass
    efficiency-weighted one-block quantile reinsert only on the prob27-like
    2-bay concentrated high-proc heavy-tail slice.

Metadata:
    version_id: reboot_v146_20260621_0215_prob27like_efficiency_shortlist_on_v142
    parent_version: reboot_v142_20260620_1548_prob40like_broad_move_narrow_selector_on_v136
    status: candidate
    timestamp: 2026-06-21 02:15 KST
    strategy:
        - Preserve v142 unchanged outside the target subtype.
        - Build the exact trusted v142 warm start first.
        - On the target subtype only, rank tardy blocks by an efficiency score
          that combines tardiness, entry delay, and preference penalty while
          discounting workload and processing time.
        - Try only the top three unique block ids with the existing bounded
          quantile single-reinsert kernel.
        - Keep only strictly better officially feasible candidates.
    hypothesis:
        After the accepted v136 one-block move, the remaining prob27-like
        residual is still a one-block problem, but the best next block is no
        longer pure top tardy. A tiny efficiency-weighted displaced shortlist
        can recover that hidden signal while staying scoreable on the 60s
        surface.
    intended_metric_target:
        - improve the prob27-like T tail
...
ACTIVE_VERSION = "reboot_v146_20260621_0215_prob27like_efficiency_shortlist_on_v142"
...
def _try_efficiency_shortlist_reinsert(
    ...
    print(
        f"[baseline_hh reboot_v146] prob27like_efficiency_shortlist instance={prob_info.get('name')} "
...
def algorithm(prob_info: dict, timelimit: float = 60) -> dict:
    ...
    headroom = _headroom(tier)
    if remaining <= reserve + headroom:
        print(
            f"[baseline_hh reboot_v146] skip_prob27like_efficiency_shortlist instance={prob_info.get('name')} "
            f"tier={tier} remaining={remaining:.2f}s reserve={reserve:.2f}s "
            f"headroom={headroom:.2f}s base_T={base_result.get('obj1')}"
        )
        return base_solution
    ...
    if v064._result_key(research_result) < v064._result_key(base_result):
        print(
            f"[baseline_hh reboot_v146] selected_prob27like_efficiency_shortlist instance={prob_info.get('name')} "
            f"T={research_result.get('obj1')} objective={research_result.get('objective')}"
        )
        return research_solution

    print(
        f"[baseline_hh reboot_v146] keep_v142_warm_start instance={prob_info.get('name')} "
