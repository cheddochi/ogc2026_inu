"""reboot_v447_20260703_trackA_prob11_shifted_bayassign_groupmove_on_v317.py
...
Strategy:
    ...
    bay-assignment seed from v445 and replace the unstable replay/polish tail
    with ejection-chain / companion groupmove candidates on top of that seed.
"""
...
ACTIVE_VERSION = "reboot_v447_20260703_trackA_prob11_shifted_bayassign_groupmove_on_v317"
...
def algorithm(prob_info: dict, timelimit: float = 60) -> dict:
    ...
    if budget <= 0.0:
        print(
            f"[baseline_hh reboot_v447] keep_warm instance={prob_info.get('name')} "
    ...
    if len(cluster) < 3:
        print(
            f"[baseline_hh reboot_v447] no_cluster instance={prob_info.get('name')} "
    ...
    for label, cluster_plan, chooser in v420._plan_specs(prob_info, assignments, cluster):
        ...
        for variant_label, variant_ids in variants[:2]:
            ...
            search_local_ids = list(dict.fromkeys(variant_ids + rescue_local_ids[:2]))
            for sequence_mode in ("backward_latest", "latest", "slack"):
                ...
                if candidate_result.get("feasible") and _score_key(candidate_result) < _score_key(best_result):
                    best_label = base_label
                    best_solution = candidate_solution
                    best_result = candidate_result

                if not candidate_result.get("feasible") or time.time() >= deadline:
                    continue

                search_candidates = v357._migration_candidates(
                    prob_info,
                    assignments,
                ...
                if time.time() < deadline:
                    search_candidates += v357._companion_groupmove_candidates(
                        prob_info,
                        assignments,
                        candidate_assignments,
                        cluster_plan,
                ...
                move_log.append((base_label + "_search_local_ids", [int(block_id) for block_id in search_local_ids]))
                for sublabel, candidate_assignments2 in search_candidates:
                    if time.time() >= deadline:
                        break
                    candidate_solution2, candidate_result2 = v357._evaluate_candidate(prob_info, candidate_assignments2)
                    tag = f"{base_label}_{sublabel}"
                    _append_attempt(attempted, tag, candidate_result2)
                    if candidate_result2.get("feasible") and _score_key(candidate_result2) < _score_key(best_result):
                        best_label = tag
                        best_solution = candidate_solution2
                        best_result = candidate_result2
        break
    ...
    if time.time() < deadline and _score_key(best_result) < _score_key(warm_result):
        ...
        _append_attempt(attempted, "best_shifted_bayassign_groupmove_plus_warmrepair", repaired_result)
        move_log.append(
            ("best_shifted_bayassign_groupmove_plus_warmrepair", [int(block_id) for block_id, _, _ in accepted_moves])
        )
        if repaired_result.get("feasible") and _score_key(repaired_result) < _score_key(best_result):
            best_label = "best_shifted_bayassign_groupmove_plus_warmrepair"
            best_solution = repaired_solution
            best_result = repaired_result

    print(
        f"[baseline_hh reboot_v447] prob11_shifted_bayassign_groupmove instance={prob_info.get('name')} "
