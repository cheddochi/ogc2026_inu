...
══════════════════════════════════════════════════════════════
  ...
  제약조건
    ...
    (C3) blk[v,k] ≥ x[k,b] + Σ_{a∈B(b)} x[v,a] - 1
         ∀ (v,k) s.t. dest[v]>dest[k],  ∀ b ∈ parking
         B(b) = {a ∈ parking : a가 b→EXIT 경로 위에 있는 노드}

         ★ 집약 재정식화 이유:
           차량 v는 정확히 1개 노드에 배치되므로
           Σ_{a∈B(b)} x[v,a] ∈ {0,1} 임을 이용
           원래 (v,k,a,b) 4중 반복 → (v,k,b) 3중 반복으로 압축
           제약 수: N_VP × N_NP → N_VP × N_parking  (약 6배 감소)
"""
...
# B[b] = b를 막을 수 있는 노드들의 집합  (집약 C3에서 사용)
from collections import defaultdict
B_lookup = defaultdict(list)
for (a, b) in blocking_node_pairs:
    B_lookup[b].append(a)

c3_count = sum(1 for (v, k) in blocking_veh_pairs for b in parking if B_lookup[b])
print(f"  C3 제약 수 : {c3_count}  (집약 전: {len(blocking_veh_pairs)*len(blocking_node_pairs)})")
...
with gp.Env() as env, gp.Model("RoRo_Stowage", env=env) as m:
    ...
    # (C3) 블로킹 선형화 — 집약 버전 (무료 라이선스 2,000개 제한 대응)
    #
    #   원래 공식: blk[v,k] ≥ x[v,a] + x[k,b] - 1  for each (a,b) blocking pair
    #   집약 공식: blk[v,k] ≥ x[k,b] + Σ_{a∈B(b)} x[v,a] - 1  for each b
    #
    #   동치 증명:
    #     차량 v는 정확히 1개 노드에 있으므로 Σ_{a∈B(b)} x[v,a] ∈ {0,1}
    #     k가 b에 있고(x[k,b]=1) v가 B(b) 내 어떤 a에 있으면(Σ=1) → RHS=1 → blk=1 강제
    #     그 외 경우 → RHS ≤ 0 → 제약 비활성
    #     ∴ 원래 공식과 완전히 동치
    for (v, k) in blocking_veh_pairs:
        for b in parking:
            blockers = B_lookup[b]   # b를 막는 노드 리스트
            if blockers:
                m.addConstr(
                    blk[v, k] >= x[k, b] + gp.quicksum(x[v, a] for a in blockers) - 1,
                    name=f"C3_{v}_{k}_{b}"
                )
