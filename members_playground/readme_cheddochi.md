...
- 병렬 branch-and-bound (`Threads=4`, 4코어 환경 최적화)
...
| `Threads` | `4` (_MAX_THREADS) | CPU 4코어 제한 |
| `MIPGap` | `0.01` | 1% 최적성 갭에서 조기 종료 |
| `Presolve` | `2` | 공격적 전처리 |
| `Cuts` | `2` | 공격적 컷 생성 |
| `Symmetry` | `2` | 대칭성 공격적 제거 → 탐색 공간 축소 |
| `Heuristics` | `0.3` | 탐색 시간의 30%를 MIP 휴리스틱에 할당 (기본 5%) |
| `RINS` | `10` | 10노드마다 RINS 실행 → 현재 해 집중 개선 |
| `MIPFocus` | `1` | 초기 실현 가능 해 확보 우선 |
| `ImproveStartTime` | `tlimit × 0.2` | 타임리밋 20% 이후 개선 집중 모드 전환 |
| `NodefileStart` | `_MAX_MEM_GB × 0.8` = 11.2 GB | RAM 최대 활용 후 디스크 기록 |
| `SoftMemLimit` | `_MAX_MEM_GB` = 14 GB | Gurobi 10+: 메모리 초과 시 중단 |
...
| 파라미터 | 값 | 의미 |
|---|---|---|
| `num_search_workers` | `4` (_MAX_THREADS) | CPU 4코어 병렬 탐색 |
| `max_memory_in_mb` | `14 × 1024` | RAM 14 GB 제한 |
| `linearization_level` | `2` | LP 완화 강화 → 타이트한 하한 → 빠른 수렴 |
...
**주요 Gurobi repair 파라미터**

| 파라미터 | 값 | 의미 |
|---|---|---|
| `Threads` | `4` (_MAX_THREADS) | CPU 4코어 제한 |
| `MIPFocus` | `1` | 실현 가능 해 조기 확보 우선 |
| `Heuristics` | `0.5` | 시간의 50%를 휴리스틱에 할당 — 소형 MIP에서 빠른 feasible 해 확보 |
| `NodefileStart` | `_MAX_MEM_GB × 0.8` | RAM 최대 활용 |
...
while deadline - time.time() > 0.5:
  1. k개 블록 선택 (destroy — worst-first 또는 random, 50/50)
  2. repair: _gurobi_repair() 또는 _greedy_repair()
  3. 개선되면 accept, 아니면 거절
  4. k 조정 (adaptive)
```

**Destroy 전략: Worst-first + Random 혼합**

```python
if random.random() < 0.5:
    destroy = _worst_blocks(prob_info, current, k, n)  # 지연 기여도 상위 블록 우선
else:
    destroy = random.sample(range(n), k)               # 순수 무작위
```

`_worst_blocks()`는 지연(`exit_time - due_date`) 기여도가 높은 블록을 `k//2`개 우선 선택하고
나머지는 무작위로 보충한다.
worst-first는 이터레이션당 목적 개선 폭을 높이고,
random 혼합은 국소해 고착 방지를 위한 탐색 다양성을 유지한다.

**내부 복사: `copy.deepcopy` → 경량 dict 복사**

```python
_sched_copy = lambda s: {bi: dict(v) for bi, v in s.items()}
```

schedule 값이 모두 정수/문자열 primitives이므로 shallow copy로 충분하다.
`copy.deepcopy` 대비 n=500 기준 약 5~10배 빠르다.
...
**환경 제약 상수** (4코어 CPU / 16 GB RAM)

| 상수 | 값 | 적용 대상 | 의미 |
|---|---|---|---|
| `_MAX_THREADS` | `4` | Gurobi, CP-SAT | 모든 솔버의 스레드 수 상한 |
| `_MAX_MEM_GB` | `14` | Gurobi, CP-SAT | 솔버 메모리 상한 (GB). 16GB 중 OS·Python용 2GB 여유 |
...
myalgorithm.algorithm(prob_info, timelimit)
  └─ casat_cheddochi.algorithm(prob_info, timelimit)
        ...
        │    ├─ _cpsat_mip()        §6
        │    └─ _adaptive_lns()     §9  ←─ _worst_blocks()   (destroy 전략)
        │                                ←─ _gurobi_repair()  §8
        │                                    └─ _greedy_repair() §7
        └─ _finalize()
              ├─ _spatial()         §10
...
*마지막 수정: 4코어 CPU / 16 GB RAM 환경 제약 내 성능 최적화 — Gurobi Symmetry·Heuristics·RINS 추가, CP-SAT linearization_level=2, LNS worst-first destroy 혼합 + 경량 dict 복사*
